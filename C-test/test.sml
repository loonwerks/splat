...
datatype result = EXACT of string | SHORT of string | EOS;
...
let open TextIO
    ...
    fun checkFile () =
      case getchars(istrm,12)
       of EOS => true
        | EXACT V => if test (drop_nl line) then checkFile() else false
        | SHORT V => test (drop_nl line)  (* expected to be false, cuz short *)
