...
`(rulesWithSymInRhs sym [] = []) /\
 (rulesWithSymInRhs sym ((rule l r)::rst) =
  if MEM sym r then ((l, breakAtElem sym r)::rulesWithSymInRhs sym rst)
...
`` ~(lhsWithLastSym sym (rule s l::rs)=[]) ==>
 (~(lhsWithLastSym sym [rule s l] = [])
    \/ ~(lhsWithLastSym sym rs = []))``,
SRW_TAC [] [] THEN
Cases_on `l` THENL[
...
` ~(lhsWithLastSym sym (rule s l::rs)=[]) ==>
 (~(lhsWithLastSym sym [rule s l] = [])
   \/ ~(lhsWithLastSym sym rs = []))` by METIS_TAC [lwls_r1] THEN
RES_TAC  THEN
`~(lhsWithLastSym  sym [rule s l] = []) ==>
...
If L=L(G) for some CFG G = (V,T,P,S), then L-{e} is L(G') for a CFG G' with no
useless symbols or e-productions.
*)
...
``!s. FINITE s ==> !g. (s = LIST_TO_SET (rules g))
...
`{nt | isNonTmnlSym nt /\ MEM nt l'} =
 {nt | isNonTmnlSym nt /\ nt IN (LIST_TO_SET l')}` by SRW_TAC [] [Once SET_TO_LIST_IN_MEM] THEN
...
`FINITE (LIST_TO_SET l')` by METIS_TAC [finiteLists] THEN
SRW_TAC [] [rule_terminals] THEN
`{tmnl | isTmnlSym tmnl /\ MEM tmnl l'} =
 {tmnl | isTmnlSym tmnl /\ tmnl IN (LIST_TO_SET l')}`
   by SRW_TAC [] [SET_TO_LIST_IN_MEM] THEN
 POP_ASSUM SUBST_ALL_TAC THEN
`{tmnl | isTmnlSym tmnl /\ tmnl IN (LIST_TO_SET l')} SUBSET (LIST_TO_SET l')`
...
Definition nullableML :
  (nullableML g sn [] = T) /\
  (nullableML g sn [TS ts] = F) /\
  (nullableML g sn [NTS A] =
    (if (MEM (NTS A) sn) then
        F
     else EXISTS (nullableML g (NTS A :: sn))
                 (getRhs A (rules g)))) /\
  (nullableML g sn (s::ss) = (nullableML g sn [s] /\ nullableML g sn ss))
Termination
 WF_REL_TAC `inv_image (measure (λ(g,sn). CARD (nonTerminals g DIFF LIST_TO_SET sn))
                         LEX
                        measure LENGTH)
                        (λ(g,sn,syms).((g,sn),syms))` >>
 SRW_TAC [] []
 >- (`FINITE (nonTerminals g)` by METIS_TAC [finiteNtsList,FINITE_LIST_TO_SET] >>
     `FINITE (set sn)` by METIS_TAC [FINITE_LIST_TO_SET] >>
     `FINITE (NTS A INSERT set sn)` by METIS_TAC [FINITE_INSERT] >>
     FULL_SIMP_TAC (srw_ss()) [CARD_DIFF] >>
     SRW_TAC [] []
      >- (SRW_TAC [] [CARD_INSERT,FINITE_LIST_TO_SET] >>
          `~(getRhs A (rules g) = [])` by METIS_TAC [listNotEmpty] >>
          `(NTS A) IN (nonTerminals g)` by METIS_TAC [ntsInGr] >>
          `(nonTerminals g INTER (NTS A INSERT set sn)) =
           (NTS A INSERT set sn) INTER nonTerminals g`
             by METIS_TAC [INTER_COMM] >>
      ...
      >- (`~(getRhs A (rules g) = [])` by METIS_TAC [listNotEmpty] >>
          `(NTS A) IN (nonTerminals g)` by METIS_TAC [ntsInGr] >>
          `~((NTS A) IN set sn)` by FULL_SIMP_TAC (srw_ss()) [MEM,LIST_TO_SET] >>
          `~(NTS A IN (nonTerminals g INTER set sn))` by METIS_TAC [IN_INTER] >>
          `~(nonTerminals g = set sn)` by METIS_TAC [] >>
          `FINITE (nonTerminals g)` by METIS_TAC [finiteNtsList,FINITE_LIST_TO_SET] >>
          `FINITE (set sn)` by METIS_TAC [FINITE_LIST_TO_SET] >>
          `FINITE (NTS A INSERT set sn)` by METIS_TAC [FINITE_INSERT] >>
          `(CARD (nonTerminals g) - CARD (nonTerminals g INTER set sn)) =
           (CARD ((nonTerminals g) DIFF (set sn)))` by METIS_TAC [CARD_DIFF] >>
          POP_ASSUM SUBST_ALL_TAC >>
          `(NTS A) IN (nonTerminals g DIFF set sn)`
             by (FULL_SIMP_TAC (srw_ss()) [DIFF_DEF] THEN METIS_TAC []) >>
          `~((nonTerminals g DIFF set sn)={})` by METIS_TAC [MEMBER_NOT_EMPTY] >>
          `~(CARD (nonTerminals g DIFF set sn)=0)`
              by METIS_TAC [CARD_EQ_0,FINITE_DIFF] >>
          decide_tac)
    )
 >- decide_tac
End
...
val R = ``inv_image
            (measure (λ(g,sn). CARD ((nonTerminals g) DIFF (LIST_TO_SET sn)))
                      LEX
             measure (λsyms : symbol list. LENGTH syms))
            (λ(g,sn,syms).((g,sn),syms))``
...
val listderiv_def = Define`
  listderiv R d s0 s1 <=> rtc2list R d /\ (HD d = s0) /\ (LAST d = s1)`;
...
THENL[
      ...
      THENL[
            ...
            Cases_on `dl` THEN FULL_SIMP_TAC (srw_ss()) [rtc2list]
            ]])
...
val nullableMLEq = store_thm (
  ...
  Cases_on `h` THEN SRW_TAC [] [nullableML']
  THENL[
    SRW_TAC [boolSimps.DNF_ss][isNonTmnlSym_def],
...
val nullableImpML = store_thm (
  ...
  THENL[
	...
	`(derives g)^* l []`
        ...
        `derivNts d INTER set sn = {}`
           by SRW_TAC [][] THEN
        `~(d=[])` by METIS_TAC [rtc2list,listderiv_def]
        THENL[
