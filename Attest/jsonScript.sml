...
open Lib HolKernel bossLib Theory Parse Tactic boolLib intLib stringLib;

val _ = new_theory "json";

open arithmeticTheory listTheory ASCIInumbersTheory;
...
(*---------------------------------------------------------------------------*)
(* toNum = ASCIInumbers$num_from_dec_string                                  *)
(*---------------------------------------------------------------------------*)
...
Definition toInt_def : 
 toInt s = 
   ...
   if HD s = #"-"
    then SOME (-(int_of_num(toNum (TL s))))
    else SOME   (int_of_num(toNum s))
End

Definition getNum_def : 
  getNum s =
    let (digits,ss) = SPLITL (\c. isDigit c \/ c = #"-") s
    in case toInt digits
        of SOME i => SOME (numLit (Int i),ss)
         | NONE => NONE
End

Definition getKeyword_def :
 getKeyword s =
   let (word,ss) = SPLITL isAlpha s
   in 
     if word = "null" then SOME (nullLit,ss) else
     if word = "true" then SOME (boolLit T, ss) else
     if word = "false" then SOME (boolLit F, ss) 
     else
       NONE
End

(*---------------------------------------------------------------------------*)
(* Leading " has already been seen                                           *)
(*---------------------------------------------------------------------------*)
