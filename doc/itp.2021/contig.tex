...
\section{In-message assertions}

Given the matching algorithm and the prescence of boolean expressions,
the building blocks for expressing predicates on messages are in
place. A simple approach is to add a new ``assertion'' constructor to
...
\]
%
The semantics of \konst{Assert} can already be expressed
with the notion of an empty record, and $\emptyset$, the empty
language.
%
\[\konst{Assert}\; P = \konst{Union} \; (P, \{\,\}) \ (\neg P, \emptyset) \]
%
The implementation mirrors this closely
%
\[
...
illustrative purposes) assertion language over R-values.

\begin{example}[$A^n B^n C^n$]

It is well known that $A^n B^n C^n$ is not a context-free
language. The \verb+Assert+ type can be used to specify something
quite close to this language, namely $u \cdot A^{\konst{val}(u)} \cdot B^{\konst{val}(u)} \cdot C^{\konst{val}(u)}$.
