...
datatype segs = SEGS of (lval * locval) list;

fun add_seg x (SEGS list) = SEGS(x::list);
  ...
  of Basic a =>
     let val awidth = atomicWidths a
       ...
       in case tdrop awidth s
           of NONE => NONE
            | SOME (segment,rst) =>
               SOME(add_seg (path, constrFn segment) segs,
                    rst, Redblackmap.insert(WidthValMap,path,segment))
       end
   ...
   | Raw exp =>
       ...
       in
         ...
         of NONE => NONE
          | SOME (segment,rst) =>
              SOME(add_seg (path,BLOB segment) segs,
                   rst, Redblackmap.insert(WidthValMap,path,segment))
       end
   | Scanner scanFn =>
      (case scanFn s
        of NONE => raise ERR "segFn" "Scanner failed"
         | SOME(segment,rst) =>
              SOME(add_seg (path,SCANNED segment) segs,
                   rst, Redblackmap.insert(WidthValMap,path,segment)))
   | Recd fields =>
...
fun pp_locval locval =
 let open PP
     fun prepare(s) = Lib.quote(String.toString s)
 in case locval
     of BOOL s => block CONSISTENT 1
            [add_string "Bool", add_string(prepare s)]
      | CHAR s => block CONSISTENT 1
            [add_string "Char", add_string(prepare s)]
      | INT (i,s) => block CONSISTENT 1
            [add_string ("i"^Int.toString (i*8)), add_string(prepare s)]
      | UINT (i,s) => block CONSISTENT 1
            [add_string ("u"^Int.toString (i*8)), add_string(prepare s)]
      | FLOAT s => block CONSISTENT 1
            [add_string "Float", add_string(prepare s)]
      | DOUBLE s => block CONSISTENT 1
            [add_string "Double", add_string(prepare s)]
      | ENUM (s1,s2) => block CONSISTENT 1
            [add_string ("Enum"^" "^s1), add_string(prepare s2)]
      | BLOB s => block CONSISTENT 1
            [add_string "Blob", add_string(prepare s)]
      | SCANNED s => block CONSISTENT 1
            [add_string "Scanned", add_string(prepare s)]
 end
;

fun pp_segs (SEGS segs) =
 let open PP
     fun pp_pair(lval,locval) =
        block CONSISTENT 0
          [pp_lval lval, add_string " := ", pp_locval locval, NL]
 in
    block CONSISTENT 1
       ([add_string "[" ] @ map pp_pair (rev segs) @ [add_string "]"])
...
val _ = PolyML.addPrettyPrinter (fn d => fn _ => fn locval => pp_locval locval);
val _ = PolyML.addPrettyPrinter (fn d => fn _ => fn segs => pp_segs segs);
